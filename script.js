    ...
    if (typeof io !== 'undefined') {
       ...
       socket.on('connect_error', (error) => {
        ...
        socket.on('call-rejected', (data) => {
            const reason = data && data.message ? data.message : 'Call was declined';
            alert(reason);
...
async function initiateCall(friendId, type) {
    try {
        // Show call UI immediately so user sees ringing state while permissions are requested
        const callInterface = document.getElementById('callInterface');
        callInterface.classList.remove('hidden');
        document.querySelector('.call-channel-name').textContent = `Calling...`;

        // Always include a video track so screen sharing can replace it later
        // For audio calls we disable the video track after acquisition
        ...
        const reason = error && (error.message || error.name) ? `${error.name || ''} ${error.message || ''}`.trim() : 'Permission denied or device not found';
        alert(`Failed to access camera/microphone: ${reason}. Please allow access in the browser (lock icon → Site settings) and ensure no other app is using the devices.`);
    }
}
...
async function acceptCall(caller, type) {
    try {
        // Request only what we need (video only for video calls)
        const constraints = {
            video: type === 'video',
            audio: true
        };
...
function createPeerConnection(remoteSocketId, isInitiator) {
    ...
    pc.ontrack = (event) => {
        ...
        // Attach stream (fallback to build stream from track if event.streams is empty)
        const incomingStream = (event.streams && event.streams[0]) ? event.streams[0] : (() => {
